...
SET @lastPerson = (SELECT MAX(joindate) FROM Members);

SELECT firstname, surname
  FROM Members
 WHERE joindate = @lastPerson
...
--I need to filter the bookings by tennis courts (0,1) and then merge it with the members based on the memid. I also need the name of the facility from the facilitiy table

SELECT DISTINCT CONCAT(f.name,' ' ,m.surname,' ', m.firstname) 
	AS Court_Bookings
  FROM Bookings as b
